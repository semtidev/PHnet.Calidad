...
<body class="hold-transition locked-page">
    ...
    <div class="lockscreen-item">
        ...
        <img src="{{ asset('dist/img/users/' . Auth::user()->photo . '.jpg') }}" alt="User Image">
        </div>
        <!-- /.lockscreen-image -->
    ...
    <div class="text-center">
        <b><a href="{{ route('logout') }}" class="text-black" onclick="event.preventDefault(); localStorage.setItem('sesionstatus', 'active'); document.getElementById('logout-form').submit();">Inicie Sesi&oacute;n con otra cuenta</a></b>
    ...
    @auth
        localStorage.setItem('sesionstatus', 'locked');
    @endauth
    @guest
        localStorage.setItem('sesionstatus', 'active');
        window.location = '/phnet.calidad/public';
    @endguest
    ...
    function sendUnlocked() {
        ...
        $.ajax({
            type: 'POST',
            url: '/phnet.calidad/public/api/unlocked',
            data: formData,
            cache: false,
            contentType: false,
            processData: false,
            success: function(response)
            {
                if (response.success) {
                    localStorage.setItem('sesionstatus', 'active');
                    document.location = '/phnet.calidad/public/';
                }
                else {
                    $('#formfields').addClass('error');
                    $('#avatar').addClass('avatar-error');
                    loading.html('Acceso Denegado');
                }
            },
            error: function(xhr){
                document.location = '/phnet.calidad/public/locked';
            }
        });
    }
    </script>
